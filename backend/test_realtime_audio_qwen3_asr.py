"""
实时音频捕获 + Qwen3-ASR-1.7B 流式转写测试

功能：
1. 实时捕获 Windows 系统音频（浏览器播放的视频、音乐等）
2. 使用 Qwen3-ASR-1.7B 模型进行流式转写
3. 实时显示字幕效果
4. 支持 52 种语言和方言
5. 启用 CUDA 加速

依赖安装：
pip install pyaudiowpatch numpy qwen-asr[vllm] torch

使用方法：
python test_realtime_audio_qwen3_asr.py
...
class RealtimeQwen3ASRStreaming:
    """实时音频流式转写（Windows 系统音频捕获 + Qwen3-ASR）"""
    
    def __init__(
        self,
        sample_rate: int = 16000,
        chunk_duration: float = 3.0,  # 每个块的时长（秒），Qwen3-ASR 建议 2-5 秒
        model_name: str = "Qwen/Qwen3-ASR-1.7B",
        backend: str = "transformers",  # Windows 推荐使用 transformers
        device: str = "cuda",
        use_modelscope: bool = True  # 优先使用 ModelScope 下载
    ):
        """
        初始化实时音频流式转写
        
        Args:
            sample_rate: 采样率（Hz），Qwen3-ASR 支持 16000Hz
            chunk_duration: 每个音频块的时长（秒），建议 2-5 秒
            model_name: 模型名称
                - "Qwen/Qwen3-ASR-1.7B" (推荐，最高准确率)
                - "Qwen/Qwen3-ASR-0.6B" (轻量级，更快)
            backend: 后端类型，"transformers" (推荐，Windows 兼容) 或 "vllm" (Linux)
            device: 运行设备（cuda 或 cpu）
            use_modelscope: 是否优先使用 ModelScope 下载（国内更快）
        """
    ...
    def _load_model(self):
        ...
        logger.info("=" * 80)
        logger.info("提示：首次运行会自动从 ModelScope 下载模型（约 3-4 GB），请耐心等待...")
        logger.info("模型会缓存到 ~/.cache/modelscope/hub/")
        logger.info("=" * 80)
        
        # 设置 ModelScope 为默认下载源
        os.environ['MODELSCOPE_CACHE'] = os.path.expanduser('~/.cache/modelscope')
        
        # 使用 transformers 后端（Windows 兼容）
        self.model = Qwen3ASRModel.from_pretrained(
            self.model_name,
            dtype=torch.bfloat16,
            device_map=self.device,
            max_inference_batch_size=1,  # 实时模式使用单个批次
            max_new_tokens=2048,
        )
        
        logger.info(f"✅ {self.model_name} 模型加载完成")
        logger.info(f"块大小: {self.chunk_size} 采样点 = {self.chunk_duration:.2f} 秒")
        logger.info(f"支持 52 种语言和方言，自动识别")
        logger.info(f"实时转写模式：累积音频 + 增量显示")
        logger.info("=" * 80)
    ...
    def _audio_processing_thread(self):
        ...
        try:
            ...
            self._clear_screen()
            print("=" * 80)
            print("📺 实时字幕（Qwen3-ASR-1.7B 多语言识别）")
            print("=" * 80)
            print()
                ...
                try:
                    ...
                    try:
                        ...
                        results = self.model.transcribe(
                            ...
                            return_time_stamps=False,  # 实时模式不需要时间戳
                        )
                        ...
                        if results and len(results) > 0:
                            ...
                            detected_lang = result.language if hasattr(result, 'language') else "unknown"
                            ...
                            if text.strip():
                                ...
                                should_newline = (
                                    len(current_text) >= 50 or  # 超过 50 字换行
                                ...
                                # 性能信息（调试用）
                                if chunk_index % 5 == 0:
                                    logger.debug(f"处理性能: {elapsed*1000:.0f}ms, 音频能量: {audio_energy:.4f}, 语言: {detected_lang}")
    ...
    def _refresh_display_with_current(self, all_sentences, current_line, current_time, current_lang, max_lines):
        ...
        if current_line:
            current_text = "".join(current_line)
            print(f"[{current_time}] [{current_lang}] {current_text} ...")  # 加 ... 表示还在继续
...
def main():
    """主函数"""
    print("\n" + "╔" + "═" * 78 + "╗")
    print("║" + " " * 18 + "实时音频流式转写测试" + " " * 18 + "║")
    print("║" + " " * 12 + "Qwen3-ASR-1.7B + CUDA 加速" + " " * 12 + "║")
    print("║" + " " * 15 + "支持 52 种语言和方言" + " " * 15 + "║")
    print("╚" + "═" * 78 + "╝\n")
    
    # 创建实时转写器
    streaming = RealtimeQwen3ASRStreaming(
        sample_rate=16000,
        chunk_duration=3.0,  # 3 秒一个块，平衡延迟和准确率
        model_name="Qwen/Qwen3-ASR-1.7B",  # 使用 1.7B 模型，最高准确率
        backend="vllm",  # 使用 vLLM 后端，性能最佳
